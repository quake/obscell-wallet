...
impl ContractDeployer {
    ...
    pub fn are_all_deployed(&self, info: &DeployedContracts) -> Result<bool, String> {
        Ok(self.is_deployed(&info.stealth_lock)? && self.is_deployed(&info.ckb_auth)?)
    }

    /// Deploy all contracts (stealth-lock and ckb-auth).
    ///
    ...
    pub fn deploy_all(&self) -> Result<DeployedContracts, String> {
        ...
        {
            println!(
                "Contracts already deployed - stealth-lock tx: 0x{}, ckb-auth tx: 0x{}",
                hex::encode(info.stealth_lock.tx_hash.as_bytes()),
                hex::encode(info.ckb_auth.tx_hash.as_bytes())
            );
            return Ok(info);
        ...
        // Generate blocks to confirm ckb-auth deployment before deploying stealth-lock
        println!("Generating blocks to confirm ckb-auth...");
        self.generate_blocks(4)?;
        // Small delay to let indexer sync
